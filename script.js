...
function generateProblem() {
  ...
  switch (activeMode) {
    ...
    // UPDATED: always 8-bit → 2-digit hex
    case MODES.B2H: {
      const val = randomInt(0, 255);                  // full byte
      const bin = padLeft(toBinary(val), 8);          // 8 bits
      const hex = padLeft(toHex(val), 2);             // 2 hex digits
    ...
    // UPDATED: always 2-digit hex → 8-bit binary
    case MODES.H2B: {
      const val = randomInt(0, 255);                  // full byte
      const hex = padLeft(toHex(val), 2);             // 2 hex digits
      const bin = padLeft(toBinary(val), 8);          // 8 bits
    ...
    // UPDATED: Mixed Hex uses same full-byte style for bin↔hex
    case MODES.MIXED_HEX: {
      const val    = randomInt(0, 255);               // full byte 0–255
      ...
      if (choice === 0) {
        ...
        // Binary → Hex (8 bits → 2 hex digits)
        expr   = `${bin} → ? in Hex`;
        answer = hex;
        type   = 'hex';
      } else {
        // Hex → Binary (2 hex digits → 8 bits)
        expr   = `${hex} → ? in Binary`;
        answer = bin;
        type   = 'binary';
      }
...
function attachEvents() {
  ...
  // Allow clicking on overlay background to close
